...
import medusa from '../medusa'; // Path to your central JS-SDK file
...
export default function Checkout() {
  ...
  // 2. The combined submission function (Now correctly exposed directly to Checkout)
  const handleStripeSubmit = async (event, stripe, elements) => {
    ...
    try {
      ...
      // Create the cart on the backend server
      ...
      // Force Fulfillment binding
      const { shipping_options } = await medusa.store.fulfillment.listCartOptions({ cart_id: cart.id });
      if (shipping_options && shipping_options.length > 0) {
        await medusa.store.cart.addShippingMethod(cart.id, { option_id: shipping_options[0].id });
      }

      // Fetch the updated, fully populated cart structure
      const refreshed = await medusa.store.cart.retrieve(cart.id, { fields: "+total,+items,+shipping_methods" });
      const currentCart = refreshed.cart;

      // Initialize the Stripe Session on Medusa's backend
      const { payment_collection } = await medusa.store.payment.initiatePaymentSession(
        currentCart,
        { provider_id: "pp_stripe_stripe" }
      );

      const backendSecret = payment_collection?.payment_sessions?.[0]?.data?.client_secret;
      if (!backendSecret) {
            ...
            <table>
             ...
             {total > 0 ? (
                <div style={{ border: '1px solid #ccc', padding: '20px', marginTop: '20px' }} className='stripe-component'>
                  ...
                  >
                    {/* THIS IS THE ONLY PART THAT CHANGED BELOW */}
                    <EmbeddedStripeForm 
